...
router.post("/login", async (req, res) => {
  try {
    ...
    const user = await User.findOne({ username: username }); //finds a user that matches the entered email

    if (!user)
      return res
        .status(400)
        .json({ msg: "no account with this username has been created" });
    ...
    if (!isMatch)
      return res
        .status(400)
        .json({ msg: "the password you entered is not correct" });
...
router.get("/", auth, async (req, res) => {
  ...
  res.json({
    //makes sure we dont display user email and password in the console
    username: user.username,
    id: user._id,
  });
});
